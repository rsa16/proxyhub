  ...
  class ChromeTabs {
    ...
    setTabCloseEventListener(tabEl) {
      tabEl.querySelector('.chrome-tab-close').addEventListener('click', _ => {
        console.log(tabEl);
        this.emit("preCloseTab", { tabEl });
        if ($(".newTab").length)
        {
          for (let i=0; i < chromeTabs.tabEls.length; i++)
          {
            if (chromeTabs.tabEls[i] == tabEl)
            {
              if ($(".newTab").attr("id").startsWith(i.toString()))
              {
                console.log("remove");
                $(".newTab").remove();
    ...
    setCurrentTab(tabEl) {
      ...
      let isSwitchingFromNewTab = false;  
      let isSwitchingToNewTab = false;
      if (chromeTabs.activeTabEl.children[2].children[1].innerHTML == 'New Tab')
      {
        isSwitchingFromNewTab = true;
      }
      if (tabEl.children[2].children[1].innerHTML == 'New Tab')
      {
        isSwitchingToNewTab = true;
      }
      ...
      for (let i = 0; i < this.tabEls.length; i++)
      {
        if (this.tabEls[i] == tabEl) 
        {
          ...
          $(`#${i.toString()}`).css("display", "block");
          if (isSwitchingFromNewTab && (!isSwitchingToNewTab))
          {
            $(".pageContent").css("display", "block");
          }
        } else {
